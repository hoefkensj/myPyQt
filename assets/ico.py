#!/usr/bin/env python
import uuid,base64,re

def iconname(ename):
	rex=re.compile(r'(?P<FIRST>.)(?P<ICO>.*?)(?P<CAP2>[A-Z])',re.VERBOSE)
	res=rex.search(ename)
	return '{FIRST}{ICO}'.format(FIRST=res['FIRST'], ICO=res['ICO'])


icolib					=		{
	'Search'  : [ b'' ,
                b''] ,
	'>'    		:	[ b'' ,
                b''],
	'<'   		:	[ b'',
                b''],
	'Inc'     : [ b'PHN2ZyB3aWR0aD0iMTYiIGhlaWdodD0iMTYiIGVuYWJsZS1iYWNrZ3JvdW5kPSJuZXciIHZlcnNpb249IjEuMSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KIDxyZWN0IHg9IjIiIHk9IjciIHdpZHRoPSIxMSIgaGVpZ2h0PSIxIiBmaWxsPSIjMzYzNjM2Ii8+Cjwvc3ZnPgo=',
                b''],
	'Dec'     :	[ b'',
                b''],
	'Edit'    :	[ b'',
                b''],
	'Copy'    : [ b'',
                b''],
	'Reg'   	:	[ b'',
                b'']
	}




# Base64 decode the dictionary
decoded_dict = {key: base64.b64decode(val[0]) for key, val in icolib.items()}
[print(str(decoded_dict[key]).split('\\n')[1:-1]) for key in decoded_dict]
decoded_dict = {key: base64.b64decode(val[1]) for key, val in icolib.items()}
[print(str(decoded_dict[key]).split('\\n')[1:-1]) for key in decoded_dict]
# Extract the data according to the provided scheme
search = {'path': {}}
search['path']['d'] = decoded_dict['Search'].decode()
search['path']['fill'] = "{COLOR}"
inc = {'rect': {
'x':"2" ,
'y':"7",
'width':"11",
'height':"1",
'fill'	:'{COLOR}'}}
# edit= {'path': {'d' : 'm3 8h10v1h-10z',  'color' : '#363636',  'fill' : '#363636',  'overflow' : 'visible',  'stroke-width' : '.70711', }
# You can now use the `search` dictionary as desired

# '''
# <svg width="16" height="16" enable-background="new" version="1.1" xmlns="http://www.w3.org/2000/svg">
# <rect x="2" y="7" width="11" height="1" fill="#dedede"/>
# <rect transform="rotate(90)" x="2" y="-8" width="11" height="1" fill="#dedede"/>
# </svg>
# '''